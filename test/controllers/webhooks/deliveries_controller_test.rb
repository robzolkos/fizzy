...
class Webhooks::DeliveriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index preloads cards for comment events" do
    webhook = Webhook.create!(
      board: boards(:writebook),
      name: "Comment deliveries",
    ...
    comment_event = Event.create!(
      ...
      board: webhook.board,
      creator: users(:david),
      eventable: comments(:layout_overflowing_david),
      action: "comment_created"
    )
    second_comment_event = Event.create!(
      account: webhook.account,
      board: webhook.board,
      creator: users(:david),
      eventable: comments(:shipping_1),
      action: "comment_created"
    )

    Webhook::Delivery.create!(webhook: webhook, event: comment_event, state: :completed)
    Webhook::Delivery.create!(webhook: webhook, event: second_comment_event, state: :completed)

    assert_queries_match(/FROM [`"]cards[`"].* IN \(/, count: 1) do
      get board_webhook_deliveries_path(webhook.board, webhook), as: :json
      assert_response :success
    end
  end
